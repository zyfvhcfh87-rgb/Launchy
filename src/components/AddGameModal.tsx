...
export const AddGameModal: React.FC<AddGameModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setError("");
    onAddGame({ title, exePath, args, artworkPath });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 backdrop-blur-sm p-4">
      <div className="w-full max-w-lg bg-bgCard border border-slate-800 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 bg-slate-900/60 border border-slate-800 rounded-xl text-white placeholder-slate-600 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 text-sm"
              required
            />
          </div>
          ...
          <div>
            <label className="block text-xs uppercase font-bold text-textMuted mb-1.5 tracking-wider">
              Executable Path (.exe) <span className="text-red-500">*</span>
            </label>
          ...
          <div className="pt-4 border-t border-slate-800/40 flex justify-end space-x-3">
            <button
              type="button"
              onClick={onClose}
            ...
            <button
              type="submit"
              className="px-5 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white text-sm font-bold rounded-lg shadow-lg shadow-indigo-900/40 transition-all select-none flex items-center space-x-2"
            >
