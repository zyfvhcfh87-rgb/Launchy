...
pub fn launch(game: &Game) -> Result<(), String> {
    let install_path = game.install_path.as_ref()
        .ok_or_else(|| "Install path is missing for itch.io game".to_string())?;

    let exe_name = game.launch_exe.as_ref()
        .ok_or_else(|| "Launch executable is missing for itch.io game".to_string())?;
    ...
    cmd.spawn()
        .map_err(|e| format!("Failed to spawn itch.io standalone executable: {}", e))?;
